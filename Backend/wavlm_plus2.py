...
'''
NOTE: 
Speaker가 wavlm인 경우 (args.speaker == 'wavlm'): WavLMEncoderLayer2 활용, 추가로 parameter freeze되도록 WavLMWrapper 내부에 작성 
Speaker가 wavlm아닌 경우 (단일 모델로서 활용되는 경우, 학습시키고자 할때): WavLMEncoderLayer 활용; PEFT & adaptor방식 활용
'''
...
class WavLMEncoderLayer2(nn.Module):
    def __init__(self, config, has_relative_position_bias: bool = True):
        super().__init__()
        self.attention = wavlm.WavLMAttention(
            embed_dim=config.hidden_size,
...
class WavLMWrapper(nn.Module):
    def __init__(
        ...
        self.model_config.speaker                = args.speaker
        
        # 3. Config encoder layers with adapter or embedding prompt
        # pdb.set_trace()
        if self.args.speaker == 'wavlm': 
            print("frozen wavlm is used for speaker emb")
            self.backbone_model.encoder.layers = nn.ModuleList(
                [WavLMEncoderLayer2(self.model_config, has_relative_position_bias=(i == 0)) for i in range(self.model_config.num_hidden_layers)]
            )
        else: 
            print("PEFT for wavlm")
            self.backbone_model.encoder.layers = nn.ModuleList(
        ...
        or self.args.finetune_method == "finetune" or self.args.finetune_method == "lora" \
        or self.args.finetune_method == "combined" or self.args.speaker == 'wavlm': 
    ...
    def forward(self, x, length=None):
        ...
        # ).hidden_states ## 모든 hidden state에서 결과 나옴 
