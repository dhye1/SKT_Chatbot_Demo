...
class TextAudioClassifier(nn.Module):
    def __init__(self, audio_model=None, speaker_model=None, text_model=None, 
                 audio_dim=512, text_dim=None, hidden_dim=256, num_classes=2, dropout_prob=0.5, 
                 speaker='None', speaker_dim=10, cross_modal_atten=False, modal='audio',
                 phq_mode='symptom', num_symptom=8):
        ...
        if self.phq_mode in ['symptom']: 
            self.phq8_classifiers = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(self.input_dim, self.hidden_dim),
                    nn.BatchNorm1d(self.hidden_dim),
                    nn.ReLU(),
                    nn.Dropout(self.dropout_prob),
                    nn.Linear(self.hidden_dim, 1)  # Binary classification for each symptom
                ) for _ in range(num_symptom)
            ])
        
        if self.phq_mode in ['depression']: 
            ...
            self.phq8_binary_classifier = nn.Sequential(
                nn.Linear(self.hidden_dim, self.hidden_dim),
                nn.BatchNorm1d(self.hidden_dim),
                nn.ReLU(),
                nn.Dropout(self.dropout_prob),
                nn.Linear(self.hidden_dim, 1)  # Binary classification for depression
            )
            
            self.W_d = nn.Linear(num_symptom, num_symptom, bias=False)  # W_d is a learnable weight
            self.b_d = nn.Parameter(torch.zeros(1))  # b_d is a learnable bias
    ...
    def forward(self, audio_input, text_input=None, speaker_ID=None, length=None):
        if self.modal == 'audio':
            ...
            feature = self.text_model(embeddings=text_input)
        elif self.modal in ['multimodal', 'multimodal_concat']:
            if self.cross_modal_atten:
                ...
                elif self.modal == 'multimodal_concat':
        ...
        hidden_outputs = []
        if self.phq_mode in ['symptom']:
            for classifier in self.phq8_classifiers:
                hidden_output = classifier[:-1](feature)  # Get output up to the second last layer
                hidden_outputs.append(hidden_output)
                symptom_output = classifier[-1](hidden_output)  # Apply the last layer for binary output
                symptom_outputs.append(symptom_output)
            symptom_outputs = torch.stack(symptom_outputs, dim=1)  
            hidden_outputs = torch.stack(hidden_outputs, dim=1)  # Shape: [batch_size, num_symptom, hidden_dim] 
            
        if self.phq_mode in ['depression']:
            for classifier in self.phq8_classifiers:
                hidden_output = classifier(feature)  # Get output up to the second last layer
                hidden_outputs.append(hidden_output)
            hidden_outputs = torch.stack(hidden_outputs, dim=1)  # Shape: [batch_size, num_symptom, hidden_dim] 
            
            mean_symptom_outputs = hidden_outputs.mean(dim=-1)  # Corrected from symptom_outputs to hidden_outputs
            
            weights = torch.softmax(self.W_d(mean_symptom_outputs) + self.b_d, dim=1)            
            
            weights = weights.unsqueeze(-1)
            weighted_sum = torch.sum(weights * hidden_outputs, dim=1)  # Weighted sum of hidden outputs
            depression_output = self.phq8_binary_classifier(weighted_sum)
            return hidden_outputs, depression_output

        return symptom_outputs
    ...
    def initialize_weights(self):
        ...
        if self.phq_mode in ['depression']:
            for layer in self.phq8_binary_classifier:
                if isinstance(layer, nn.Linear):
                    nn.init.kaiming_normal_(layer.weight, mode='fan_out', nonlinearity='relu')
                    if layer.bias is not None:
                        nn.init.constant_(layer.bias, 0)
            ...
            if hasattr(self, 'W_d'):
                nn.init.kaiming_normal_(self.W_d.weight, mode='fan_out', nonlinearity='relu')
