...
def count_class_instances(input_file_list):
    """
    Count the number of instances for each symptom class in the dataset.
    :param input_file_list:        Input data entries [key, filepath, labels]
    :return class_counts:          Dictionary with counts for each symptom class
    """
    class_counts = {
        'phq_nointerest': 0,
        'phq_depressed': 0,
    ...
    for data in input_file_list:
        if data[-1] == 'train':  # Only count instances from the training data
            phq_labels = data[4]  # Assuming data[4] contains phq labels
            class_counts['phq_nointerest'] += phq_labels[0]
            class_counts['phq_depressed'] += phq_labels[1]
            class_counts['phq_sleep'] += phq_labels[2]
            class_counts['phq_tired'] += phq_labels[3]
            class_counts['phq_appetite'] += phq_labels[4]
            class_counts['phq_failure'] += phq_labels[5]
            class_counts['phq_concentrating'] += phq_labels[6]
            class_counts['phq_moving'] += phq_labels[7]
            class_counts['phq_binary'] += data[5]  # Assuming data[5] contains binary label
...
def set_finetune_dataloader(
    ...
    if args.dataset == 'daic' and symptom_class_weight is None or label_class_weight is None:
        ...
        symptom_weights = torch.tensor([
            class_counts['phq_nointerest'],
            class_counts['phq_depressed'],
            class_counts['phq_sleep'],
            class_counts['phq_tired'],
            class_counts['phq_appetite'],
            class_counts['phq_failure'],
            class_counts['phq_concentrating'],
            class_counts['phq_moving']
        ]).float()
        symptom_weights = symptom_weights.sum() / symptom_weights
        symptom_weights = symptom_weights / symptom_weights.sum()
        
        if symptom_class_weight is None:
            symptom_class_weight = symptom_weights
            print(f"symptom_class_weight: {symptom_class_weight}")
            
        if label_class_weight is None:
            label_class_weight = torch.tensor([class_counts['phq_binary']]).float()
            print(f"label_class_weight: {label_class_weight}")
            
    if args.dataset == 'daic':
        data_generator = DepressionDatasetGenerator(
